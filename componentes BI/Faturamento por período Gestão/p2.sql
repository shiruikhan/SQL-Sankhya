...
SELECT PRO.AD_AGRUPADOR,
	...
	((SUM(ITE.VLRTOT - ITE.VLRDESC) + SUM(NVL(ITE.AD_VLROUTROS,0)))/(SELECT SUM(ITE3.VLRTOT - ITE3.VLRDESC) + SUM(NVL(ITE3.AD_VLROUTROS,0)) AS TOTALFAT
																					...
																					WHERE TRUNC(CAB3.DTNEG) >= :P_PER.INI AND TRUNC(CAB3.DTNEG) <= :P_PER.FIN AND CAB3.TIPMOV = 'D' AND PRO3.USOPROD = 'V' AND (CAB3.CODEMP = 501 OR CAB3.STATUSNFE <> 'D') AND ((GRU3.CODGRUPOPROD = :P_GRUPO) OR :P_GRUPO IS NULL) AND ((:P_CODEMP IS NULL) OR (CAB3.CODEMP = :P_CODEMP)) AND ((:P_CODVEND IS NULL) OR (VEN3.CODVEND = :P_CODVEND)) AND CAB3.CODTIPOPER IN (1200,1201,/*1206,*/1216,1217,1272))) * 100 AS PARTEFATURADO
FROM TGFITE ITE
