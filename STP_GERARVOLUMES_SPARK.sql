create or replace PROCEDURE STP_GERARVOLUMES_SPARK (P_NUNOTA    INT) AS FIELD_NUNOTA    NUMBER;
    V_CODPROD       INT;
    V_SEQUENCIA     INT;
    V_QTDNEG        FLOAT;
    V_CODPRODEMB    INT;
    ...
    CURSOR CUR_SOBRA IS
        SELECT 
            PED.SEQ, PED.CODPROD, PED.M3, 
            (SELECT M3 FROM TGFPRO P WHERE CODPROD = V_EMBPADRAO) AS M3EMB
        INTO V_SEQUENCIA, V_CODPROD, V_M3, V_M3EMB
        FROM AD_EMBPED PED 
        WHERE PED.NUNOTA = FIELD_NUNOTA 
            AND PED.CODPRODEMB IS NULL    
...
BEGIN
        ...
        OPEN CUR_ITENS;
        LOOP
            FETCH CUR_ITENS INTO V_CODPROD, V_QTDNEG; --V_SEQUENCIA,
        ...
        LOOP
            ...
            ELSE
				...
				OPEN CUR_SOBRA;
				LOOP
					FETCH CUR_SOBRA INTO V_SEQUENCIA, V_CODPROD, V_M3, V_M3EMB;
					EXIT WHEN CUR_SOBRA%NOTFOUND;

					SELECT PESOBRUTO INTO V_PESOBRUTO
					FROM TGFPRO
					...
					IF (V_PESOACUMULADO + V_PESOBRUTO <= 25) AND (V_M3ACUMULADO + V_M3 <= V_M3EMB) THEN
						...
						-- Acumular o peso do produto na embalagem
