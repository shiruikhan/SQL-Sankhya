...
BEGIN
    ...
    LOOP                   
        ...
        FROM TCBINT 
        WHERE NUNICO = FIELD_NUNOTA AND ORIGEM IN ('E') AND REFERENCIA >= TRUNC(SYSDATE-5,'MM');
        ...
        WHERE NUNICO IN (SELECT DISTINCT(NUFIN) FROM TGFFIN WHERE NUNOTA = FIELD_NUNOTA) 
          AND ORIGEM IN ('B','F') AND REFERENCIA >= TRUNC(SYSDATE-5,'MM');
        ...
        ELSIF P_CORRENTE = 'S' AND P_COUNT_CAB > 0 THEN
            ...
            WHERE CODEMP||'-'||REFERENCIA||'-'||NUMLANC||'-'||TIPLANC||'-'||NUMLOTE||'-'||VLRLANC||'-'||SEQUENCIA IN (
                ...
                FROM TCBINT 
                WHERE NUNICO = FIELD_NUNOTA AND ORIGEM IN ('E') AND REFERENCIA >= TRUNC(SYSDATE-5,'MM')
            ) AND REFERENCIA >= TRUNC(SYSDATE-5,'MM');
            ...
            IF P_COUNT_FIN > 0 THEN
                ...
                WHERE CODEMP||'-'||REFERENCIA||'-'||NUMLANC||'-'||TIPLANC||'-'||NUMLOTE||'-'||VLRLANC||'-'||SEQUENCIA IN (
                    SELECT CODEMP||'-'||REFERENCIA||'-'||NUMLANC||'-'||TIPLANC||'-'||NUMLOTE||'-'||VLRLANC||'-'||SEQUENCIA 
                    FROM TCBINT 
                    WHERE NUNICO IN (SELECT DISTINCT(NUFIN) FROM TGFFIN WHERE NUNOTA = FIELD_NUNOTA) AND ORIGEM IN ('B','F') AND REFERENCIA >= TRUNC(SYSDATE-5,'MM')
                ) AND REFERENCIA >= TRUNC(SYSDATE-5,'MM');
