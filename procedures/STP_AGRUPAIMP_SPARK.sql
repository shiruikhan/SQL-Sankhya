...
SELECT NVL((SELECT MAX(TO_NUMBER(NVL(AD_AGRUPADORIMP,0))) FROM TPRIPROC WHERE AD_AGRUPADORIMP IS NOT NULL),0)+1 INTO P_AGRUP FROM DUAL;
     ...
     UPDATE TPRIPROC SET AD_AGRUPADORIMP= P_AGRUP WHERE IDIPROC = FIELD_IDIPROC;
