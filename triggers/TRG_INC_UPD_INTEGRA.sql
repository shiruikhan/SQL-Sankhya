...
BEGIN
    ...
    IF :NEW.CODPARC IS NULL THEN
        ...
        BEGIN            
            ...
            INSERT INTO TGFPAR (
                CODPARC, TIPPESSOA, IDENTINSCESTAD, NOMEPARC, CGC_CPF, ATIVO, CLIENTE, CEP, CODEND, NUMEND, COMPLEMENTO,
            ...
            VALUES (
                v_codparc, SUBSTR(:NEW.TIPPESSOA, 1, 1), :NEW.IDENTINSCESTAD, UPPER(:NEW.RAZAOSOCIAL), :NEW.DOCUMENTO, 'S', 'S', :NEW.CEP, :NEW.CODEND, :NEW.NUMERO, :NEW.COMPLEMENTO,
                :NEW.CODBAI, :NEW.CODCID, :NEW.EMAIL, :NEW.EMAIL, UPPER(:NEW.RAZAOSOCIAL), CASE WHEN TRIM(:NEW.TIPPESSOA) = 'JC' THEN 'X' ELSE 'C' END, 'S', 'X', 'S', 'S', SYSDATE, SYSDATE, 'S', CASE WHEN SUBSTR(TRIM(:NEW.TIPPESSOA), 1, 1) = 'F' THEN 'N' ELSE 'A' END
            );
