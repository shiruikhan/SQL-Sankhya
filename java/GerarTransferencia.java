...
public class GerarTransferencia implements AcaoRotinaJava {
	...
	public void doAction(ContextoAcao ca) throws Exception {
		...
		BigDecimal codEmpOrig = BigDecimalUtil.valueOf((String) codEmpOrigStr);
		BigDecimal codEmpDestino = BigDecimalUtil.valueOf((String) codEmpDestinoStr);
		BigDecimal codLocalOrig = BigDecimalUtil.valueOf((String) codLocalOrigStr);
		BigDecimal codLocalDestino = BigDecimalUtil.valueOf((String) codLocalDestinoStr);
		...
		for (int i = 0; i < ca.getLinhas().length; i++) {
				...
				while (rsNotas.next()) {
					...
					try {
						...
						while (rs.next()) {
							...
							if (nunotaEntOldInt > 0 || nunotaSaiOldInt > 0) {
								retorno = "Este(s) pedido(s) j� gerou NF de Transferencia!";
							}

							if (conferido.equals("S") && retorno == "") {
								...
								try {
									...
									DynamicVO save = impDAO.create().set("NUNOTA", nunota).set("SEQUENCIA", sequencia)
								...
								retorno = "O pedido nro �nico: " + nunota + " ainda n�o foi conferido!";
			...
			// ca.setMensagemRetorno(msgErro.toString());
		}
		...
		if (retorno == "") {
			...
			try {
				...
				while (query.next()) {
					...
					if (temLancamento.equals("S")) {
						...
						try {
							...
							System.out.println("Cabe�alho incluido: " + nuNotaGerado);
							...
							while (queryIte.next()) {
								...
								System.out.println("Inicio inclus�o Produto " + queryIte.getBigDecimal("CODPROD"));
								...
								itemVO = (DynamicVO) dwfEntityFacade
								...
								System.out.println("[GerarTransferencia] - Fim inclus�o item CODPROD: "
										+ queryIte.getBigDecimal("CODPROD") + " sequencia " + sequencia);

								// INICO INCLUSAO TGFSER
								...
								while (querySerie.next()) {
									...
									System.out.println("[GerarTransferencia] - inicio da inclus�o da TGFSER "
											+ " sequencia " + sequencia);
							...
							msgRetorno = msgRetorno + e.getMessage();
							e.printStackTrace();
						}
						...
						try {
							...
							System.out.println("Cabe�alho incluido saida: " + nunotaSaida);

							// nunotaSaida = nuNotaGerado;
							...
							while (queryIte.next()) {
								...
								System.out.println("Fim inclus�o nota de saida" + queryIte.getBigDecimal("CODPROD"));
								...
								while (querySerie.next()) {
									...
									try {
										...
										@SuppressWarnings("unused")
										DynamicVO save = impDAO.create().set("NUNOTA", nunotaSaida)
												.set("CODPROD", codProd).set("SEQUENCIA", sequencia)
												.set("SERIE", controle).set("ATUALESTOQUE", atualEstoque)
												// .set("TIPCONTEST", ticontest)
												.save();
							...
							/*
							 * if (nunotaSaida.compareTo(new BigDecimal(0)) > 0) {
							 * 
							 * PersistentLocalEntity entity = dwfEntityFacade
							 * .findEntityByPrimaryKey(DynamicEntityNames.CABECALHO_NOTA, nunotaSaida);
							 * entity.remove();
							 * 
							 * }
							 */
				...
				ca.setMensagemRetorno("Nro �nico Nota de Entrada: " + nunotaEntrada + " <br> "
						+ "Nro �nico Nota de Saida: " + nunotaSaida);
			...
			ca.setMensagemRetorno(retorno);

		}

		// FINAL DA TRASNFERENCIA
